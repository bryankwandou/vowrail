...
import { POST as agentRun } from '@/app/api/agent-run/route';
...
describe('workflow API contracts', () => {
  ...
  it('runs a challenge-bound agent preflight before wallet access', async () => { const response = await agentRun(jsonRequest('http://test/api/agent-run', {})); const data = await response.json(); expect(response.status).toBe(200); expect(data.deterministic.reason).toBe('policy_passed'); expect(data.deterministic.checks).toHaveLength(6); expect(data.readyForWallet).toBe(true); expect(data.ai.role).toBe('explanation_only'); });
